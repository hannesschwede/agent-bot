...
const STREAMLIT_URL = "https://stunning-engine-8fm2wcihxrjsnffkabftyj.streamlit.app/";
...
export default function AuditPage() {
  return (
    <div className="min-h-screen bg-white text-black font-sans">
      <Navbar cta={{ href: "#audit-tool", label: "Audit starten" }} />
      <header className="bg-dark text-white">
        <div className="mx-auto max-w-6xl px-6 py-20 md:py-32">
          <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-accent mb-6">Kostenloses Tool</p>
          <h1 className="text-[48px] md:text-[80px] font-black leading-[0.9] tracking-[-0.03em] mb-6">LEAD-<br />PERFORMANCE<br /><span className="text-accent">AUDIT.</span></h1>
          ...
          <a href="#audit-tool" className="inline-block mt-10 bg-accent text-white px-8 py-4 text-sm font-bold uppercase tracking-[0.15em] hover:bg-white hover:text-dark transition-colors">Jetzt kostenlos testen</a>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-6xl flex flex-wrap">
          {[{label:"Preis",value:"€0"},{label:"Setup",value:"0 min"},{label:"Ergebnis",value:"Sofort"},{label:"Daten",value:"Harte Fakten"}].map((s) => (
            <div key={s.label} className="flex-1 min-w-[120px] border-r last:border-r-0 border-black/10 px-6 py-5 text-center">
              <span className="block text-2xl font-black text-accent">{s.value}</span>
      ...
      <section className="px-6 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <div key={f.title} className="border-2 border-black/10 p-6 hover:border-accent transition-colors">
      ...
      {/* Embedded Audit Tool */}
      <section id="audit-tool" className="px-6 py-20 md:py-28 bg-dark">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-3xl md:text-4xl font-black uppercase mb-4 text-white">Audit <span className="text-accent">starten.</span></h2>
          <p className="text-white/50 mb-8">URL eingeben, analysieren lassen, Ergebnisse lesen. Direkt hier, ohne die Seite zu verlassen.</p>
          <div className="border-2 border-white/10 bg-white overflow-hidden">
            <iframe
              src={STREAMLIT_URL}
              title="Lead-Performance Audit Tool"
      ...
      <section className="bg-dark text-white px-6 py-20 md:py-28 border-t border-white/10">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.step} className="border border-white/10 p-6 hover:border-accent transition-colors">
                <span className="text-accent text-[11px] font-bold">SCHRITT {item.step}</span>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="flex flex-wrap gap-3">
            {["Python","Streamlit","BeautifulSoup4","Streamlit Cloud"].map((tech) => (
              <span key={tech} className="border-2 border-black/10 px-4 py-2 text-sm font-semibold hover:border-accent transition-colors">{tech}</span>
            ))}
          </div>
          <p className="mt-6 text-sm text-black/40">Open Source auf <a href="https://github.com/agent-botde/Agent-Bot.de_Lead-Performance_Audit" target="_blank" rel="noopener noreferrer" className="text-accent underline hover:no-underline">GitHub</a></p>
        </div>
      </section>
      <Footer />
    </div>
